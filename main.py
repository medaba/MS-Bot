...
#     """
#     Отправляет и выводит в консоль сообщение с сырым JSON-ом на любой полученный message.
#     """
#     print(str(m.as_json()))
...
async def info(m: Message):
    await m.answer(
        "⚠️ *ДИСКЛЕЙМЕР:*\n\n"
        "Данный бот является лишь попыткой собрать в одном месте полезные, "
        "на мой взгляд, материалы, ссылки и практические советы для людей, которые столкнулись с РС. "
...
async def useful_links(m: Message):
    await m.answer(
        'В этом меню собраны ссылки на различные полезные онлайн-ресурсы о РС, упражнения для реабилитации, ' 
